...
import { createTemporaryMailbox, getMyMailboxes } from "@/actions/mailbox.actions";
import { logoutAction } from "@/actions/auth.actions";


// ✅ CORREGIDO: Tipos con Date (coinciden con Prisma)
interface Message {
  id: string;
  subject: string | null;
  from: string;
...
export default function DashboardPage() {
  ...
  // ✅ CORREGIDO: Verifica que mailbox exista
  ...
  await logoutAction();
  // No necesitas router.push porque logoutAction ya hace redirect
  };
          ...
          {loading ? (
            ...
            <div className="grid gap-4">
              {mailboxes.map((mailbox) => (
                <div key={mailbox.id} className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition border border-gray-100">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      <h3 className="font-semibold text-gray-800">{mailbox.aliasName || "Sin nombre"}</h3>
                      <p className="text-sm text-gray-600 font-mono">{mailbox.emailAddress}</p>
                      {/* ✅ CORREGIDO: createdAt ya es Date */}
                      <p className="text-xs text-gray-400">Creado: {mailbox.createdAt.toLocaleDateString()}</p>
                    </div>
                    <button
                  ...
                  {mailbox.messages.length > 0 && (
                    <div className="mt-3 text-sm text-gray-500 border-t border-gray-100 pt-2">
                      📥 {mailbox.messages.length} mensaje(s). Último: {mailbox.messages[0].receivedAt.toLocaleString()}
                    </div>
                  )}
                </div>
              ))}
